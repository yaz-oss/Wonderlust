...
export default function Register() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Could not create your account right now.')
    } finally {
  ...
  return (
    <main className="flex min-h-[calc(100vh-8rem)] items-center justify-center px-4 py-12">
      <section className="grid w-full max-w-5xl overflow-hidden rounded-lg border border-white/70 bg-white shadow-[0_30px_90px_rgba(72,18,113,0.24)] lg:grid-cols-[0.92fr_1fr]">
        <div className="relative min-h-[32rem] overflow-hidden bg-[#24135f] p-8 text-white sm:p-10">
          <div className="absolute -left-16 top-8 h-52 w-52 rounded-full bg-fuchsia-500/45 blur-2xl" />
          <div className="absolute -right-16 -top-10 h-56 w-56 rounded-full bg-cyan-400/35 blur-2xl" />
          <div className="absolute bottom-14 left-10 h-20 w-20 rounded-full bg-fuchsia-500 shadow-[0_0_40px_rgba(236,72,153,0.65)]" />
          <div className="absolute right-10 top-24 h-28 w-28 rounded-full bg-cyan-400/80 shadow-[0_0_50px_rgba(34,211,238,0.45)]" />
          <div className="relative z-10 flex h-full flex-col justify-between">
            ...
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-cyan-100">Start Exploring</p>
              <h1 className="mt-3 max-w-sm text-4xl font-bold leading-tight">Create your travel command center.</h1>
              <p className="mt-4 max-w-sm leading-7 text-white/78">
                Save destinations, book tickets, and keep every upcoming trip neatly in one place.
              </p>
            </div>
        ...
        <div className="flex items-center px-6 py-10 sm:px-12">
          <div className="w-full">
            <h2 className="text-3xl font-bold text-primary-text">Register</h2>
            <p className="mt-2 text-primary-text-soft">Create an account to book trips and manage tickets.</p>
            ...
            <form className="mt-7 space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="relative mt-6">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="bg-white px-2 text-primary-text-soft">Or sign up with</span>
                </div>
              </div>
            ...
            <p className="mt-5 text-center text-sm text-primary-text-soft">
              Already registered? <Link to="/login" className="font-semibold text-primary-accent">Login</Link>
            </p>
          </div>
        </div>
      </section>
